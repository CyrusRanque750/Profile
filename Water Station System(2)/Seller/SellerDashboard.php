...
// Query to retrieve the shop status for the seller
$query = "SELECT * FROM shopinfo WHERE seller_id = ?";
...
if ($result && $result->num_rows > 0) {
    ...
    // Debugging: Print the retrieved status
    error_log("Shop status: " . $shop['status']); // Check the status in the error log

    if ($shop['status'] === 'approved') {
        $statusMessage = "Your shop '{$shop['name']}' has been approved and is now active.";
    } elseif ($shop['status'] === 'rejected') {
        $statusMessage = "Your shop '{$shop['name']}' has been rejected. Please contact support for further details.";
    } else {
        $statusMessage = "Your shop is pending approval.";
    }
} else { 
    $statusMessage = "No shop found. Please register your shop.";
}
    ...
    <div class="content-container">
        ...
        <div class="status-message">
            <?php if ($statusMessage): ?>
                <p><?php echo htmlspecialchars($statusMessage); ?></p> <!-- Display the message directly -->
            <?php endif; ?>
        </div>

        <div class="actions">
            <button class="register-button"><a href="Shop.php">Register Shop</a></button>
